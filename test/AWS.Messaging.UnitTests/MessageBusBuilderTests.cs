...
public class MessageBusBuilderTests
{
    ...
    [Fact]
    public void MessageBus_AddLambdaMessagePoller()
    {
        _serviceCollection.AddAWSMessageBus(builder =>
        {
            builder.AddLambdaMessagePoller("queueUrl");
        });

        var serviceProvider = _serviceCollection.BuildServiceProvider();

        // Verify that a singleton MessagePumpService was added
        var lambdaMessagePump = serviceProvider.GetServices<ILambdaMessagePump>();
        Assert.NotNull(lambdaMessagePump);
        ...
        // ...of the expected type, with expected default parameters
        if (configuration is LambdaMessagePollerConfiguration lambdaConfiguration)
        {
            Assert.Equal("queueUrl", lambdaConfiguration.SubscriberEndpoint);
            Assert.Equal(10, lambdaConfiguration.MaxNumberOfConcurrentMessages);
        }
        else
        {
            Assert.Fail($"Expected configuration to be of type {typeof(LambdaMessagePollerConfiguration)}");
        }
    ...
    [Fact]
    public void MessageBus_AddLambdaMessagePoller_NonDefaultOptions()
    {
        _serviceCollection.AddAWSMessageBus(builder =>
        {
            builder.AddLambdaMessagePoller("queueUrl", options => {
                options.MaxNumberOfConcurrentMessages = 20;
            });
        });
    ...
    /// <summary>
    /// Tests that an exception is thrown when configuring the Lambda poller with invalid options
    /// </summary>
    [Fact]
    public void LambdaMessagePollerConfiguration_LambdaMessagePollerOptions_Invalid()
    {
        Assert.Throws<InvalidLambdaMessagePollerOptionsException>(() =>
            _serviceCollection.AddAWSMessageBus(builder =>
            {
                builder.AddLambdaMessagePoller("queueUrl", options =>
                {
                    options.MaxNumberOfConcurrentMessages = -3;
                });
            }));
    }
